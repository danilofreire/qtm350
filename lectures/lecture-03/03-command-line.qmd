...
- For example:
  - `ls ~/Documents/` and `ls -lh` are both valid commands that will yield (different) output
...
# Help! `r emo::ji('sos')` `r emo::ji('anguished_face')` {background-color="#2d4563"}
...
# Navigating your file system `r emo::ji('compass')` {background-color="#2d4563"}
...
## Another shell navigation exercise

:::{style="margin-top: 30px; font-size: 24px;"}

Now that you've had some practice, let's try a more challenging exercise.

1. Create a new directory called `data` in your home directory.
2. Inside the `data` directory, create three subdirectories called `raw`,
   `clean`, and `analysis`.
3. Inside the `raw` directory, create three empty files called `data1.csv`,
   `data2.csv`, and `data3.csv`.
4. Move `data2.csv` to the "clean" directory.
5. Copy `data1.csv` to the "analysis" directory.
6. Rename `data3.csv` to `data4.csv`.
7. List the contents of the `data` directory and its subdirectories.
8. Remove the `data` directory and all its contents.
...
- We will also learn about how to use [vim](https://www.vim.org/) or [neovim](https://neovim.io/) as a text editors
  - Vim is a powerful text editor that is highly configurable and can be used
    for many different programming languages. _And it is my editor of choice!_ `r emo::ji('nerd_face')`
...
# Questions? `r emo::ji('winking_face')` {background-color="#2d4563"}
