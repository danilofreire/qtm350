...
### I will show you some tips after the exercise
...
- We can also use the `less` command to scroll through the text file. This is particularly useful for large files
:::
...
- Then try to do the following:
  ...
  - View all lines from line 20 backwards in sonnets.txt
  - Count the number of lines and words in sonnets.txt
  - Find all lines in sonnets.txt that start with "But"  [[Appendix 02]{.button}](#sec-appendix-02)
:::
...
```{bash, echo=TRUE}
cat sonnets.txt | grep "Shall I compare thee" | wc
```
:::
...
```{bash, echo=TRUE}
# Find all lines in sonnets.txt that start with "But"
grep -n "But" sonnets.txt
```
